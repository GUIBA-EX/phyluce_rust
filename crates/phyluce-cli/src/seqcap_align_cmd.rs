...
pub fn run(
    ...
    if ambiguous {
        println!("NOT removing sequences with ambiguous bases...");
    } else {
        println!("Removing ALL sequences with ambiguous bases...");
    }
    ...
    for record in records {
        ...
        if ambiguous || !has_n {
            ...
            eprintln!("Skipping {locus} because it contains ambiguous bases");
        }
    }
    ...
    for locus in &locus_names {
        if loci[locus].len() < min_taxa {
            ...
            if incomplete_matrix {
                eprintln!("DROPPED locus {locus}. Too few taxa (N < 3).");
            } else {
                eprintln!("DROPPED locus {locus}. Alignment does not contain all {taxa} taxa.");
            }
        }
    }
